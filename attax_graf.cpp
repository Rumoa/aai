...
int outroJog(int jog) {
	if (jog==1) return 2; else return 1;
}

// Assinala (ou desassinala tipo=0) uma quadricula x,y
void assinala_quad(int x, int y, int atip)
{
	int col=15;	if (atip==1) col=0; else if (atip==2) col=4;
	setcolor(col); setfillstyle(1,col);	
	fillellipse(x*sq+5,y*sq+5,4,4); fillellipse(x*sq+sq-5,y*sq+5,4,4);
	fillellipse(x*sq+5,y*sq+sq-5,4,4); fillellipse(x*sq+sq-5,y*sq+sq-5,4,4);
}
...
void inicia_tabul()
{
	...
	do {
    	...
    	le_jogo(nome);
        cleardevice(); mostra_tabul(); delay(800);
		num++; if (num>NTABS) num=1;
...
int tipo_jogo(void) 
{
    ...
    do { fflush(stdin); ch=getchar(); } while (ch<'1'||ch>'4');
    return ch-48;
}
...
{
	 mostra_tabul(); delay(500); 
	 cleardevice(); setcolor(1); setbkcolor(0); settextstyle(0,0,2);
     if (venc==0) outtextxy(SIZE/2-200, SIZE/2,"Nao Ha' Vencedor! Empate!!!\n");
     else if (venc==1) outtextxy(SIZE/2-200, SIZE/2,"Venceu o Jogador 1 - Azul!");
     else outtextxy(SIZE/2-200, SIZE/2,"Venceu o Jogador 2 - Verde!");
     delay(2000); mostra_tabul(); delay(3000); 
}
...
bool movimento_valido(movimento &mov)
{
	 ...
	 if (tabul[mov.yi][mov.xi]==mov.jog && tabul[mov.yf][mov.xf]==0 && 
		 adjacente(mov, 1) ){
	 		mov.tipo=0; return true; 
		}  //expansao da peca
		
	 if (tabul[mov.yi][mov.xi]==mov.jog && tabul[mov.yf][mov.xf]==0 && 
		 adjacente(mov, 2) ){
	 		mov.tipo=1; return true; 
		}  //movimento da peca 
...
void executa_movimento(movimento mov)
{
    ...
    if (mov.tipo==1) tabul[mov.yi][mov.xi] = 0;  //movimento
    multiplica(mov);
}
  	...
  	for(int xi=0; xi<N; xi++)
  	  for(int yf=0; yf<N; yf++)
  		for(int xf=0; xf<N; xf++){
  			mov.yi=yi;mov.xi=xi;mov.yf=yf;mov.xf=xf;
			if (movimento_valido(mov)){
				copia();
  				executa_movimento(mov); 
  				int av=avalia(jog);
  				if (av>=bestav) { bestav=av; bestmov=mov; }
	...
	for(int i=0; i<N; i++) 
        for(int j=0; j<N; j++) {
     		if (tabul[i][j]== jog){
     			for(int k=-1; k<=1; k++) 
        			for(int l=-1; l<=1; l++) {
        				movs[nmovs].jog=jog; movs[nmovs].yi = i; movs[nmovs].xi = j;
        				movs[nmovs].yf = i+k; movs[nmovs].xf = j+l;
     					if (movimento_valido(movs[nmovs])) { 
						 	assinala_quad(j+l,i+k,tipoAss); nmovs++; }
     				}
     			}
     	}
	return nmovs;		
...
int jogadas_validas_pos(int jog, int xi, int yi, movimento movs[], int tipoAss)
{
	int nmovs=0;
   	if (tabul[yi][xi]== jog){
     	for(int k=0; k<N; k++) 
        	for(int l=0; l<N; l++) {
        		movs[nmovs].jog=jog; movs[nmovs].yi = yi; movs[nmovs].xi = xi;
        		movs[nmovs].yf = k; movs[nmovs].xf = l;
     			if (movimento_valido(movs[nmovs])) { assinala_quad(l,k,tipoAss); nmovs++; }
     		}
     	}
	return nmovs;	
}
...
{
	movimento movs[500];
    if (jogadas_validas(outroJog(jog), movs, 0)>0) return -1; //existem jogadas
	if (avalia(jog)>0) return jog; 
	else if (avalia(jog)<=0)return outroJog(jog);
...
void jogada_Humano(int jog)
{
	...
	do{
        int mc = mouseclick();
		if (mc == RMOUSE) {le_jogo("jogo.txt"); cleardevice(); mostra_tabul();  clearmouseclick(RMOUSE);  }
		if (mc == CMOUSE) {grava_jogo(); mostra_tabul(); clearmouseclick(519); }
		if (mc == LMOUSE) {
		    px=mousex()/sq; py=mousey()/sq;
			if (cl==0 && tabul[py][px]==jog) { 
				cl=1; mov.xi=px; mov.yi=py; 
				assinala_quad(px,py,1); 
				jogadas_validas_pos(jog, px, py, movs,1);
			}
			else if (cl==1){ 
				cl=0; mov.xf=px; mov.yf=py; 
...
void jogada(int n, int jog, int tJog){
     if (n%2 == 1) { if (tJog<=2) jogada_Humano(jog); else jogada_PC(jog,n);  }
     else { if (tJog==1 || tJog==3) jogada_Humano(jog); else jogada_PC(jog,n); }
}
...
int main(void)
{
  ...
  do {
    nMovs++; jog=outroJog(jog);      // incrementa jogada e troca de jogador
